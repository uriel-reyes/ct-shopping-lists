...
var _package = require('../../dist/package-933e64f5.cjs.dev.js');
...
async function run() {
  ...
  cli.command('start', usageStart).usage("\n\n  ".concat(usageStart)).alias('dev').action(async options => {
    ...
    const startCommand = shouldUseExperimentalBundler ? await Promise.resolve().then(function () { return require('../../dist/start-vite-3966d625.cjs.dev.js'); }) : await Promise.resolve().then(function () { return require('../../dist/start-6b8d264b.cjs.dev.js'); });
  ...
  cli.command('build', usageBuild).usage("\n\n  ".concat(usageBuild)).option('--build-only', '(optional) If defined, the command only creates the production bundles without compiling the "index.html".', {
    ...
    const buildCommand = shouldUseExperimentalBundler ? await Promise.resolve().then(function () { return require('../../dist/build-vite-e7a6315c.cjs.dev.js'); }) : await Promise.resolve().then(function () { return require('../../dist/build-9bd2f410.cjs.dev.js'); });
  ...
  cli.command('login', usageLogin).usage("\n\n  ".concat(usageLogin)).action(async options => {
    ...
    // Do this as the first thing so that any code reading it knows the right env.
    process.env.NODE_ENV = 'production';
    const loginCommand = await Promise.resolve().then(function () { return require('../../dist/login-67e038da.cjs.dev.js'); });
  ...
  cli.command('config:sync', usageConfigSync).usage("\n\n  ".concat(usageConfigSync)).option('--dry-run', '(optional) Executes the command but does not send any mutation request.', {
    ...
    // Do this as the first thing so that any code reading it knows the right env.
    process.env.NODE_ENV = 'production';
    const configSyncCommand = await Promise.resolve().then(function () { return require('../../dist/config-sync-52f1daf8.cjs.dev.js'); });
