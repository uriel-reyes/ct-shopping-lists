...
export type TBuilderMapStateFunction<Model> = (state: Partial<Model>) => Partial<Model>;
...
export type TPropertyFieldUpdater<Model> = (fnOrValue: string | TBuilderMapStateFunction<Model>) => TPropertyBuilder<Model>;
...
export type TFieldUpdater<OriginalModel, Value> = <Model = OriginalModel>(fnOrValue: TBuilderMapStateFunction<OriginalModel> | TBuilder<Model> | TBuilder<Model>[] | Value) => TBuilder<OriginalModel>;
export type TFieldBuilderArgs<Model> = {
    omitFields?: (keyof Model)[];
    keepFields?: (keyof Model)[];
};
export type TBuilder<OriginalModel> = {
    [K in keyof Required<OriginalModel>]: TFieldUpdater<OriginalModel, OriginalModel[K]>;
} & {
    build<TransformedModel>(args?: TFieldBuilderArgs<OriginalModel>): TransformedModel;
    buildGraphql<TransformedModel>(args?: TFieldBuilderArgs<OriginalModel>): TransformedModel;
    buildRest<TransformedModel>(args?: TFieldBuilderArgs<OriginalModel>): TransformedModel;
};
export type TDefaultTransformer<TransformerType extends TTransformType, Model> = 'default' extends TransformerType ? {
    default: TTransformer<Model>;
} : never;
export type TGraphqlTransformer<TransformerType extends TTransformType, Model> = 'graphql' extends TransformerType ? {
    graphql: TTransformer<Model>;
} : never;
export type TRestTransformer<TransformerType extends TTransformType, Model> = 'rest' extends TransformerType ? {
    rest: TTransformer<Model>;
} : never;
